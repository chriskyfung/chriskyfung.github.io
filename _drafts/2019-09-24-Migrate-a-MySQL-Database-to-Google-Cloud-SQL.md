...
You need to create a new Google Cloud SQL instance to host the migrated database.
...
This method export the entire source database and import the dump data to replica, typically requires downtime for the database during the entire process to keep data in sync. 
- **External replica promotion migration**
This process through the automated migration workflow provided by Goolge Cloud SQL, which aims to minimize the database uptime by promoting the replica database to replace the source after the data is synchronized.

In a production environment, you may consider the automated workflow to take its advantage. In this article, however, I will show you the **export/import** approach because it is simpler. I want to make sure you can accomplish this challenge lab.
...
Navigate to ***SQL***, click the name of the sql instance to open the details. Choose **DATABASES** tab, and click **Create database**. In the dialog, enter `wordpress` as the name and select `utf8mb4` as the character set.

![Add wordpress database to Cloud SQL](/images/posts/qwiklabs/qwiklabs-GSP306-step6-create-databases-in-cloud-sql.png)

Click **IMPORT** button at the top of the page, click **Browse** to select the dump file in the Cloud Storage and select `wordpress` in the Database field. Click **Import**.
...
ls
```
You should find there is a file called `wp-config.php`
...
```
Refresh the Demo Blog Site, the website becomes fail to display because the wordpress server does not have a database connection.
...
The WordPress site connects the Cloud SQL. It should be restored, if you refresh the Demo Blog Site in your web browser.

If the Demo Blog Site does not show, use the following command to restart the web server,
